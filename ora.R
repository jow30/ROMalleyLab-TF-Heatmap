...
if (top_key_type == 0 & is.null(gsmap) & file.exists(bg) == FALSE) {
  ...
  stop("Please check your gene identifiers. If your gene identifiers do not match any key type in the species database, you can provide a custom gene set mapping file (gsmap) with the same key type as your query genes for enrichment analysis, and provide a custom background gene list (a text file with one gene ID per line, with the same key type as your query genes) for the enrichment analysis.")
} else if (top_key_type == 0 & !is.null (gsmap) & file.exists(bg) == FALSE) {
  stop("None of the query genes match any key type in the species database, but a custom gene set mapping file (gsmap) is provided. Enrichment analysis will be performed based on the gsmap file instead of the species database. Please also provide a custom background gene list (a text file with one gene ID per line, with the same key type as your query genes) for the enrichment analysis.")
} else if (top_key_type == 0 & is.null (gsmap) & file.exists(bg) == TRUE) {
  stop("None of the query genes match any key type in the species database, but a custom background gene list is provided. Enrichment analysis will be performed based on the custom background gene list instead of the species database. Please also provide a custom gene set mapping file (gsmap) with the same key type as your query genes for enrichment analysis.")
} else if (top_key_type == 0 & !is.null (gsmap) & file.exists(bg) == TRUE) {
  message("None of the query genes match any key type in the species database, but both a custom gene set mapping file (gsmap) and a custom background gene list are provided. Enrichment analysis will be performed based on the gsmap file and the custom background gene list instead of the existing species database.")
  key_type <- NULL # key type is not applicable when using custom gsmap and background gene list instead of the species database for enrichment analysis
} else {
...
if (!is.null(gsmap)) {
  ...
  if (sum(unique(gsmap[[2]]) %in% bg_genes) == 0) {
    stop("None of the genes in the gene set mapping file (gsmap) match the background gene set. Please check your gsmap file and background gene set.")
  } else {
    message("Using gene set mapping file (gsmap) for enrichment analysis. ", sum(unique(gsmap[[2]]) %in% bg_genes), " out of ", length(unique(gsmap[[2]])), " unique genes in the gsmap file match the background gene set.")
