...
goi_tb <- goi_tb[goi_tb[[1]] %in% bg_genes,] # make sure ranked genes of interest are in the background gene set

gsea <- function(ranked_query_gene_list, species_db = NULL, gsmap = NULL, key_type = NULL, gene_set = NULL, pvalue_cutoff = 0.05, fdr_cutoff = 0.1, minGSSize = 5, maxGSSize = 10000, out_file = NULL) {
  if (!is.null(gsmap)) {
    if (!is.null(gene_set)) {
  ...
  ego <- dplyr::filter(ego, p.adjust < fdr_cutoff)
